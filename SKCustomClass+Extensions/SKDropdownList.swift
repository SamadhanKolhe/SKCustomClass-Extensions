...
public final class SKDropdownList: UIView {
    
    
    typealias CompletionHandler = (_ index:Int,_ item:String) -> Void
    func ViewList(_ view: AnyObject, data:[String],completionHandler: @escaping CompletionHandler)
    {
        SKCustom().setViewBorder(self)
        self.dataSource = data
        self.selectionAction = {(index, item) in
            completionHandler(index,item)
            print(self.indexForSelectedRow ?? "")
            self.selectRowAtIndex(0)
            self.selectionBackgroundColor = self.backgroundColor!
        }
        self.anchorView = view as? UIView
...
//        }
    ...
    public static weak var VisibleDropDown: SKDropdownList?
     ...
     */
    public dynamic var textFont = UIFont.systemFont(ofSize: 15) {
        didSet { reloadAllComponents() }
    }
    ...
    public var dismissMode = DismissMode.onTap {
        willSet {
            if newValue == .onTap {
                let gestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(SKDropdownList.dismissableViewTapped))
...
private extension SKDropdownList {
...
extension SKDropdownList {
    ...
    public func show() -> (canBeDisplayed: Bool, offscreenHeight: CGFloat?) {
        if self == SKDropdownList.VisibleDropDown {
            return (true, 0)
        }
        
        if let visibleDropDown = SKDropdownList.VisibleDropDown {
            visibleDropDown.cancel()
        }
        
        SKDropdownList.VisibleDropDown = self
        ...
        visibleWindow?.addUniversalConstraints(format: "|[SKDropdownList]|", views: ["SKDropdownList": self])
    ...
    public func hide() {
        if self == SKDropdownList.VisibleDropDown {
             ...
             */
            SKDropdownList.VisibleDropDown = nil
        }
...
extension SKDropdownList: UITableViewDataSource, UITableViewDelegate {
...
extension SKDropdownList {
    ...
    fileprivate func startListeningToKeyboard() {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(SKDropdownList.keyboardUpdate),
            name: NSNotification.Name.UIKeyboardDidShow,
            object: nil)
        NotificationCenter.default.addObserver(
            self,
